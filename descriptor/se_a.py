...
class DescrptSeA (DescrptSe):
    ...
    def __init__ (self, 
        ...
        for tt in exclude_types:
            ...
            #self.exclude_types.add((tt[1], tt[0])) #cjx mod
        print("---exclude_types after assert---")  #cjx mod
        print(self.exclude_types)   #cjx mod
        print("---------------------")  #cjx mod
    ...
    def build (self, 
               ...
               atype_ : tf.Tensor,
               natoms : tf.Tensor,
               natoms_banidx : tf.Tensor,   #cjx mod
               box_ : tf.Tensor, 
               mesh : tf.Tensor,
               input_dict : dict, 
               reuse : bool = None,
        ...
        if not self.type_one_side and type_embedding is None:
            for type_i in range(self.ntypes):
                ...
                inputs_i = tf.slice (inputs,
                                     ...
                                     [-1, natoms_banidx[2+type_i]* self.ndescrpt] )
                inputs_i = tf.reshape(inputs_i, [-1, self.ndescrpt])
                layer, qmat = self._filter(tf.cast(inputs_i, self.filter_precision), type_i, name='filter_type_'+str(type_i)+suffix, natoms=natoms, reuse=reuse, trainable = trainable, activation_fn = self.filter_activation_fn, is_banish= True)
                layer = tf.reshape(layer, [tf.shape(inputs)[0], natoms_banidx[2+type_i] * self.get_dim_out()])
                qmat  = tf.reshape(qmat,  [tf.shape(inputs)[0], natoms_banidx[2+type_i] * self.get_dim_rot_mat_1() * 3])
                output.append(layer)
                output_qmat.append(qmat)
                start_index += natoms_banidx[2+type_i]
                #------------------------------------------------------------------
                inputs_i = tf.slice (inputs,
                                     [ 0, start_index*      self.ndescrpt],
                                     [-1, (natoms[2+type_i]-natoms_banidx[2+type_i])* self.ndescrpt] )
                inputs_i = tf.reshape(inputs_i, [-1, self.ndescrpt])
                layer, qmat = self._filter(tf.cast(inputs_i, self.filter_precision), type_i, name='filter_type_'+str(type_i)+suffix, natoms=natoms, reuse=reuse, trainable = trainable, activation_fn = self.filter_activation_fn, is_banish= False)
                layer = tf.reshape(layer, [tf.shape(inputs)[0], (natoms[2+type_i]-natoms_banidx[2+type_i]) * self.get_dim_out()])
                qmat  = tf.reshape(qmat,  [tf.shape(inputs)[0], (natoms[2+type_i]-natoms_banidx[2+type_i]) * self.get_dim_rot_mat_1() * 3])
                output.append(layer)
                output_qmat.append(qmat)
                start_index += natoms[2+type_i]-natoms_banidx[2+type_i]                
#--------------cjx mod end-----------------------------------------
        else :
            inputs_i = inputs
            inputs_i = tf.reshape(inputs_i, [-1, self.ndescrpt])
    ...
    def _filter(
            ...
            name='linear', 
            reuse=None,
            trainable = True,  #cjx mod
            is_banish = False): #cjx mod
        ...
        if all_excluded:
            ...
            return result, qmat
        if is_banish:                #cjx mod
            print("banish it")         #cjx mod
            natom = tf.shape(inputs)[0]  #cjx mod
            result = tf.cast(tf.fill((natom, outputs_size_2, outputs_size[-1]), 0.), GLOBAL_TF_FLOAT_PRECISION)  #cjx mod
            qmat = tf.cast(tf.fill((natom, outputs_size[-1], 3), 0.), GLOBAL_TF_FLOAT_PRECISION)        #cjx mod
            return result, qmat   #cjx mod
